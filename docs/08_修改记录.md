...
- 时间：2026-02-01 03:33 (UTC+08:00)
- 修改范围：`src/envs/astra_env.py`、`experiments/train_hi_ppo.py`、`src/utils/explainability.py`、`src/algorithms/hi_ppo/trainer.py`、`src/algorithms/hi_ppo/rollout.py`
- 修改原因：训练无 episode 结束导致统计为空，同时奖励解释未使用配置权重，主导因素偏向 utilization。
- 修改后的效果/影响：
  - 环境增加 `episode_length` 与内部计步，按步数截断 episode，episodes/rewards 可统计。
  - 训练脚本读取 `reward` 权重与 `episode_length`，并传入环境。
  - 可解释性统计使用实际奖励权重计算贡献与主导因素。
- 验证/结果：未运行（需重新训练以生成新的 episode 统计）。

- 时间：2026-02-01 03:54 (UTC+08:00)
- 修改范围：`src/algorithms/hi_ppo/networks.py`、`src/algorithms/hi_ppo/trainer.py`、`src/algorithms/reward_functions.py`、`src/envs/astra_env.py`
- 修改原因：跨域切分数固定导致奖励几乎恒定；utilization 长期主导，策略缺少可变性。
- 修改后的效果/影响：
  - 策略网络支持 STOP 标记提前结束切分，允许使用更少域与可变跨域切分数。
  - 放置解码忽略 STOP 后的切分点，避免固定 4 次切分。
  - 奖励引入均衡度与切分惩罚：利用率按均衡度缩放，通信成本随切分比例增大。
  - 奖励计算器接入配置的域数，未使用域会影响均衡度。
- 验证/结果：未运行（需重新训练评估收敛与奖励分布）。

- 时间：2026-02-01 04:03 (UTC+08:00)
- 修改范围：`src/utils/astra_adapter.py`
- 修改原因：新策略允许停止切分，部分 NPU 可能无节点导致 Astra-sim 解析空 trace 崩溃。
- 修改后的效果/影响：为无节点的 NPU 自动注入最小 `idle_compute` 节点，避免空 trace 引发 SIGABRT。
- 验证/结果：未运行（等待重新训练验证）。
...
- 时间：2026-02-01 00:53 (UTC+08:00)
- 修改范围：`src/algorithms/hi_ppo/networks.py`
- 修改原因：PPO 训练在 MPS 上出现 in-place 修改导致的 autograd 报错，需回退切分点更新为非 in-place。
- 修改后的效果/影响：切分点更新恢复为 clone+赋值，避免 MPS 反向传播报错。
- 验证/结果：使用 mock 模式 CPU 快速训练（256 步、2 epochs、关闭 eval/save）可正常完成。

- 时间：2026-02-01 01:45 (UTC+08:00)
- 修改范围：`configs/algo/hi_ppo.yaml`
- 修改原因：训练日志提示 GraphSAGE 不支持 edge features，与实际使用的模型不一致。
- 修改后的效果/影响：将默认 GNN 模型切换为 GAT，避免 edge features 警告并符合预期模型设置。
- 验证/结果：未运行（配置更新）。

- 时间：2026-02-01 05:40 (UTC+08:00)
- 修改范围：`src/algorithms/reward_functions.py`、`src/envs/astra_env.py`、`src/utils/explainability.py`、`src/utils/types.py`、`experiments/train_hi_ppo.py`、`src/algorithms/hi_ppo/trainer.py`、`src/algorithms/hi_ppo/rollout.py`
- 修改原因：训练出现退化解（单域放置），需按文档引入约束惩罚与固定域数的均衡度定义，避免策略退化并提升解释性一致性。
- 修改后的效果/影响：
  - 奖励函数新增约束惩罚（最少启用域数、均衡度下限、低带宽跨域切分惩罚），并支持配置化。
  - 均衡度与放置统计使用固定域数，单域放置不再获得满分均衡。
  - 解释性摘要新增约束惩罚项，训练日志/rollout 传入总域数。
  - mock 运行参数补齐域数与每层运行时间。
- 验证/结果：未运行（需重新训练评估收敛与奖励分布）。

- 时间：2026-02-01 05:58 (UTC+08:00)
- 修改范围：`src/algorithms/reward_functions.py`
- 修改原因：解释性统计中的约束惩罚未体现 constraint_weight，贡献占比与真实奖励不一致。
- 修改后的效果/影响：约束惩罚先按 constraint_weight 加权后再参与奖励与解释性统计。
- 验证/结果：未运行（逻辑调整）。
